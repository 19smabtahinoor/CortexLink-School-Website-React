...
function TeachersProfile() {
  ...
  const { id } = useParams()
  useEffect(() => {
    const teaachers = AllTeachers.TeachersList.find((element) => element.id === parseInt(id))
    setImage(teaachers.image);
    setName(teaachers.name);
    setPhoneNo(teaachers.phoneNo);
    setAddress(teaachers.address);
    setEmail(teaachers.email);
    setAbout(teaachers.about);
    setExpertise(teaachers.expertise);
    setEducation(teaachers.education);
  }, [id]);
  return (
    <>
      <Wrapper>
